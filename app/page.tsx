    ...
    <main>
      <section className="hero" id="top">
        <nav className="nav container" aria-label="Main navigation">
          ...
          <a className="nav-cta" href="#membership">Book a class <span>↗</span></a>
        </nav>
      ...
      <section className="departments container" id="classes">
        ...
        <div className="schedule-callout"><span className="circle-arrow">↗</span><p>Not sure where to start? <strong>Find the class that fits your energy.</strong></p><a href="#membership">Browse the schedule</a></div>
      </section>
      ...
      <section className="pricing" id="membership"><div className="container"><div className="pricing-top"><div><p className="eyebrow red"><i /> Come as you are</p><h2>Start your<br /><em>Femme era.</em></h2></div><p>Choose the rhythm that feels right. No contracts. Just your next move.</p></div>
        <div className="intro-offer"><div className="best-ribbon">BEST VALUE</div><div><p className="eyebrow">Your first step</p><h3>The First Timer<br />Special</h3><p className="offer-copy">Three classes. Two weeks.<br />One very good decision.</p></div><div className="offer-price"><small>Just</small><b>$39</b><small>Valid for 14 days</small></div><a className="button button-light" href="mailto:thekkc.net@gmail.com?subject=Femme%20Intro%20Offer">Claim your offer <span>→</span></a></div>
        <div className="plan-grid">{plans.map((plan) => <article className={`plan ${plan.vip ? "vip" : ""}`} key={plan.name}>{plan.vip && <div className="vip-label">✦ MOST FEMME</div>}<p className="plan-name">{plan.name}</p><p className="plan-price">{plan.price}<small>/mo</small></p><p className="plan-detail">{plan.detail}</p><ul>{plan.perks.map((perk) => <li key={perk}><span>✓</span>{perk}</li>)}</ul><a href="mailto:thekkc.net@gmail.com?subject=Femme%20Membership" className="plan-link">Choose {plan.name} <span>→</span></a></article>)}</div>
        <p className="drop-in">Just dropping in? <strong>$30 / class</strong> <a href="mailto:thekkc.net@gmail.com?subject=Femme%20Drop-in">Book a single class →</a></p>
      </div></section>
      ...
      <section className="instructors container"><div className="section-heading split"><div><p className="eyebrow red"><i /> Your hype women</p><h2>Meet the women<br />behind the <em>magic.</em></h2></div><p>Experienced artists, devoted teachers, and your biggest fans from the very first eight count.</p></div><div className="instructor-grid"><article><div className="portrait p-one"><span>01</span></div><h3>Jade M.</h3><p>Pole + Floorwork</p></article><article><div className="portrait p-two"><span>02</span></div><h3>Sasha L.</h3><p>Heels + Choreo</p></article><article><div className="portrait p-three"><span>03</span></div><h3>Rae D.</h3><p>Flex Appeal</p></article><article className="join-card"><span>✦</span><p>Come meet your<br /><em>new crew.</em></p><a href="#membership">Take a class →</a></article></div></section>
